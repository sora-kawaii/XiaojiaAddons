   ...
   public void processCommand(ICommandSender var1, String[] var2) throws CommandException {
      if (var2 != null) {
         if (var2.length == 0) {
            ...
            String var6 = var2[0];
            String var7 = var2.length == 1 ? "" : String.join(" ", (CharSequence[])Arrays.copyOfRange(var2, 1, var2.length));
            if (var6.equals("reconnect")) {
               ClientSocket.reconnect();
            } else if (Checker.enabled) {
               int var3;
               int var4;
               int var5;
               switch (var6) {
                  ...
                  case "commands":
                     ...
                     break;
                  case "copy":
                     Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new StringSelection(ChatLib.removeColor(var7)), (ClipboardOwner)null);
                     break;
                  case "rename":
                  ...
                  case "core":
                     ...
                     ChatLib.chat(String.format("core for current (%d %d) is: %d", var3, var5, (new Room(var3, var5, new Data("unknown", "", 0, (ArrayList)null))).core));
                     break;
                  case "debug":
                  ...
                  case "keybind":
                     switch (var2.length) {
                        ...
                        case 2:
                           ...
                           return;
                        default:
                           String var23 = String.join(" ", (CharSequence[])Arrays.copyOfRange(var2, 2, var2.length));
                           if (var2[1].equalsIgnoreCase("add")) {
                              ...
                              if (var2[1].equalsIgnoreCase("removeWithKey")) {
                                 var23 = String.join(" ", (CharSequence[])Arrays.copyOfRange(var2, 3, var2.length));
                  ...
                  case "flag":
                     (new Thread(() -> {
                        WaterSolver.reset();
                        WaterSolver.calc(Integer.parseInt(var2[1]), (Patterns.Operation)null);
                     })).start();
                     break;
                  case "sp":
                     XiaojiaAddons.mc.getNetHandler().handleParticles(new S2APacketParticles(EnumParticleTypes.SMOKE_LARGE, false, MathUtils.getX(MinecraftUtils.getPlayer()), MathUtils.getY(MinecraftUtils.getPlayer()), MathUtils.getZ(MinecraftUtils.getPlayer()), Float.parseFloat(var2[1]), Float.parseFloat(var2[2]), Float.parseFloat(var2[3]), 1.0F, 0, new int[0]));
                     break;
                  case "cp":
                  ...
                  case "tp":
                     ...
                     var4 = Integer.parseInt(var2[2]);
                     var5 = Integer.parseInt(var2[3]);
                     XiaojiaAddons.mc.thePlayer.posX = (double)var3;
                     XiaojiaAddons.mc.thePlayer.posY = (double)var4;
                     XiaojiaAddons.mc.thePlayer.posZ = (double)var5;
                     break;
                  default:
