...
public class GhostQOL extends RenderEntityESP {
   ...
   private static int llIlIIlIll(float var0, float var1) {
      float var2;
         ...
         if (SkyblockUtils.isInMist() && Configs.ShowRunicGhost) {
            ...
            while(var3.hasNext()) {
               ...
               if (var4 instanceof EntityCreeper) {
                  ...
                  if (llllllIIll((double)((EntityCreeper)var4).getHealth(), 1000000.1) > 0 && !runicGhostUUIDs.containsKey(var5)) {
   ...
   protected float interpolateRotation(float var1, float var2, float var3) {
      float var4;
      for(var4 = var2 - var1; llIlIIlIll(var4, -180.0F) < 0; var4 += 360.0F) {
      }
