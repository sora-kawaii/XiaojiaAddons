   ...
   public void onTick(TickEndEvent event) {
      if (Checker.enabled) {
         ...
         if (this.enabled) {
            ...
            try {
               if (Configs.PlasmaFluxCD != 0 && var2 - this.lastPlasma >= (long)(1000 * Configs.PlasmaFluxCD)) {
                  this.lastPlasma = var2;
                  this.queue.add("Plasma");
                  this.queue.add(ControlUtils.getHeldItemIndex() + "");
               }

               if (Configs.HealingWandCD != 0 && var2 - this.lastHealing >= (long)(1000 * Configs.HealingWandCD)) {
                  this.lastHealing = var2;
                  this.queue.add("Healing");
                  this.queue.add(ControlUtils.getHeldItemIndex() + "");
               }

               if (Configs.GyroCD != 0 && var2 - this.lastGyro >= (long)(1000 * Configs.GyroCD)) {
                  this.lastGyro = var2;
                  this.queue.add("Gyro");
                  this.queue.add(ControlUtils.getHeldItemIndex() + "");
               }
